...
Activation Map Visualization allows people to visualize how the different neural network layer pays attention to different parts of the image with respect to the control output (steering angle prediction). The Activation Map Visualization approach in this research work allows us to see how a neural network performs holistic scene understanding for the steering angle control prediction.
...
The image below shows how the fourth convolutional layer (Conv4) pays attention ahead and the lane marking (red regions) for making the high-level decision of steering the vehicle straight ahead.
